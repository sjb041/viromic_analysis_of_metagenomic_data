...
def parse_idxstats(idxstats_file, sample_name, abun_file):
    ...
    输出两个表格: 
        ...
        计算过程表, 5 列, vOTU  length  mapped  unmapped  rel_abun
    """
    ...
    # 计算总 mapped 的和，用于计算相对丰度
    total_mapped = data['mapped'].sum()
    
    # 相对丰度计算: rel_abun = mapped / sum(mapped)
    if total_mapped > 0:
        data['rel_abun'] = data['mapped'] / total_mapped
    else:
