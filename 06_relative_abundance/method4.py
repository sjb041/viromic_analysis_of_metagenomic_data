...
###################################### step2 计算 RPKM
def pileup(bbmap_pileup, bam_file, rpkm_file):
    """
    解析 samtools idxstats 输出文件, 计算 RPKM
    """
    cmd1 = f"{bbmap_pileup} in={bam_file} rpkm={rpkm_file}"
    subprocess.run(cmd1, shell=True, check=True)

###################################### step3 归一化得到相对丰度
def parse_rpkm_file(rpkm_file, prefix, output_tsv):
    """
    处理 BBMap pileup.sh 产生的 rpkm 文件

    参数:
        rpkm_file (str): 输入的 rpkm 文件路径
        output_tsv (str): 输出的 tsv 文件路径

    输出:
        一个包含 vOTU 和 rel_abun 的 TSV 文件
    """

    # 定义列名（与 pileup.sh 输出严格一致）
    columns = [
        "Name",
        "Length",
        "Bases",
    ...
    # 归一化 RPKM
    df["RPKM"] = df["RPKM"] / df["RPKM"].sum()

    # 重命名列
    df = df.rename(columns={
        "Name": "vOTU",
        "RPKM": f"rel_abun_{prefix}"
    })
...
def main():
    ...
    # BBMap/pileup.sh
    bbmap_pileup = "/home/shijiabin/opt/bbmap/pileup.sh"
    ...
    for sample in SAMPLES:
        ...
        #### step1 使用 bowtie2 , 比对每一个样本的 reads 到 votus
        # 输入
        ...
        #### step2 计算 votus 在该 reads 样本中的 RPKM
        # 输入
        bam_file = f"{dir_mapping}/{sample}.sorted.bam"
        # 输出
        dir_rpkm = "02_rpkm"
        os.makedirs(dir_rpkm, exist_ok=True)
        rpkm_file = f"{dir_rpkm}/{sample}_rpkm.tsv"
        # 计算 RPKM
        pileup(bbmap_pileup, bam_file, rpkm_file)
        ...
        # 归一化得到相对丰度
        parse_rpkm_file(rpkm_file, sample, abun_file)
