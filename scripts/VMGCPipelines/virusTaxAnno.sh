...
geno=$2                    # 输入基因组序列文件(.fasta)
outf=$3                    # 输出文件前缀
threads=30                 # 使用的线程数

# 定义数据库路径
db=/share/data2/guorc/Database/virus/20220331/tot.faa           # 蛋白质序列数据库
db_tax=/share/data2/guorc/Database/virus/20220331/tot.tax       # 蛋白质序列数据库分类信息
db_s=/share/data2/guorc/Database/virus/20220331/db_genome.fa    # 基因组序列数据库
db_s_tax=/share/data2/guorc/Database/virus/20220331/db_genome.tax  # 基因组序列数据库分类信息
...
# 合并覆盖度和长度信息表
/share/data2/guorc/script/TableTreat.py -a $outf.nucl.cvg -b $outf.nucl.len -m table_merge -o $outf.nucl.cvg
# 根据覆盖度(>75%)和相似度(>95%)过滤结果，并排序去重
...
# 将过滤后的结果与数据库分类信息合并
/share/data2/guorc/script/TableTreat.py -a $outf.nucl.cvg2 -b $db_s_tax -o $outf.nucl.cvg2 -x 1 -y 1  -m table_merge
# 提取分类信息中的科和种信息，生成种水平分类文件
