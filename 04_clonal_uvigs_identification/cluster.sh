...
if [[ "$(realpath "$seq")" == "$(realpath "$target")" ]]; then
  # 提取序列
  cut -f1 ${out}/clusters.tsv | seqkit grep -f - ${seq} > ${out}/votus.fa

  # 获取 TSV 中行数
  tsv_lines=$(wc -l < "${out}/clusters.tsv")
  # 获取 FASTA 中序列数量
  fa_records=$(seqkit stat "${out}/votus.fa" | awk 'NR==2 {print $4}')
  # 判断数量是否一致
  if [ "$tsv_lines" -ne "$fa_records" ]; then
    echo "TSV lines: $tsv_lines, FASTA sequences: $fa_records" >&2
    exit 1
  fi
fi
