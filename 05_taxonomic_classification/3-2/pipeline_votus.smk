...
# 参考文献: A metagenome-wide study of the gut virome in chronic kidney disease
# 注意,原文中的Virus-Host DB downloaded in May 2021,应该是为了要与其他两个数据库的ICTV版本匹配

# 运行: snakemake -s pipeline_votus.smk --cores 5 --use-conda

##### 待改进部分：
# 1. 原文献中聚类votu时使用75%AF，此处使用更常用的85%，是否需要照抄原文？
# 2. 目前不选择最佳匹配,建议参考代码 https://github.com/RChGO/VMGC/blob/main/Pipelines/virusTaxAnno.sh
...
rule diamond:
    input:
        protein = "votus_{type}/02_prodigal/{sample}_protein.faa"
    output:
        diamond_results = "votus_{type}/03_diamond/{sample}.tsv"
    log:
...
rule assign:
    input:
        diamond_results = "votus_{type}/03_diamond/{sample}.tsv"
    output:
        merge_results = "votus_{type}/04_classification/{sample}_merge.tsv",
        final_results = "votus_{type}/04_classification/{sample}_final.tsv"
    run:
        # 读入 diamond_results
...
rule postprocess:
    ...
    shell:
        "cp {input.final_results} {output.final_tax}"