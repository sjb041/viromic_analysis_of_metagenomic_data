...
rule genomad:
    ...
    shell:
        ...
        "genomad annotate {input.votus} {output.dir} {params.db} --lenient-taxonomy > {log} 2>&1"
...
rule assign:
    input:
        ...
        overview = "votus_{type}/03_vcontact2/{sample}/genome_by_genome_overview.csv"
    output:
        merge_taxonomy = "votus_{type}/04_assign_classification/{sample}_merge_taxonomy.tsv",
        final_taxonomy = "votus_{type}/04_assign_classification/{sample}_final_taxonomy.tsv"
    log:
        "votus_{type}/log/assign_classification/{sample}.log"
    run:
...
onsuccess:
    ...
    shell(
        ...
        for type in {types}
        do
            log_path="votus_$type/output.log"
            rm $log_path

            for sample in {samples}
            do
                (echo "---------------- $sample ----------------"; cat votus_$type/log/assign_classification/$sample.log) >> $log_path
            done
            
            cat $log_path
        done
        """.format(types=" ".join(TYPES), samples=" ".join(SAMPLES))
    )