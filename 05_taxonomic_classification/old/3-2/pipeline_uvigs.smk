...
rule diamond:
    input:
        protein = "uvigs_{type}/02_prodigal/{sample}_protein.faa"
    output:
        diamond_results = "uvigs_{type}/03_diamond/{sample}.tsv"
    log:
...
rule assign:
    input:
        diamond_results = "uvigs_{type}/03_diamond/{sample}.tsv"
    output:
        merge_results = "uvigs_{type}/04_classification/{sample}_merge.tsv",
        final_results = "uvigs_{type}/04_classification/{sample}_final.tsv"
    run:
        # 读入 diamond_results
