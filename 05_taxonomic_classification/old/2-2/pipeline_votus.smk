...
# 运行: snakemake -s pipeline_votus.smk --cores 5
...
rule cluster:
    ...
    output:
        dir = directory("votus_{type}/00_cluster/{sample}"),
        votus = "votus_{type}/00_cluster/{sample}/votus.fa"
    log:
...
rule prodigal:
    input:
        votus = "votus_{type}/00_cluster/{sample}/votus.fa"
    output:
        protein = "votus_{type}/01_prodigal/{sample}_protein.faa",
        count_orf = "votus_{type}/01_prodigal/{sample}_count_orf.txt"
    log:
...
rule hmmscan:
    input:
        protein = "votus_{type}/01_prodigal/{sample}_protein.faa"
    output:
        tblout = "votus_{type}/02_hmmscan/{sample}.tbl"
    log:
        "votus_{type}/log/hmmscan/{sample}.log"
    shell:
...
rule format:
    input:
        ...
        tblout = "votus_{type}/02_hmmscan/{sample}.tbl"
    output:
        tblout_format = "votus_{type}/02_hmmscan/{sample}_format.tbl"
    run:
...
rule assign:
    input:
        tblout_format = "votus_{type}/02_hmmscan/{sample}_format.tbl"
    output:
        assign_tax = "votus_{type}/03_assign/{sample}.tsv"
    run:
