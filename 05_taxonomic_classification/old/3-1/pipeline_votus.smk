# Prodigal + DIAMOND + Virus-Host DB(release229_2025-06-03),同时提供genomad的分类结果
...
# 运行: snakemake -s pipeline_votus.smk --cores 5 --use-conda
...
virushostdb_dir = os.path.join(db_dir, "virushostdb_release229_2025-06-03")
virushostdb_path = os.path.join(virushostdb_dir, "virushostdb")
refseq_family_path = os.path.join(virushostdb_dir, "refseq_family.tsv")
...
onstart:
    ...
    shell(
        ...
        if [ -f {virushostdb_dir}/virushostdb_configured ]; then
            echo "Virus-Host DB 数据库已配置, 位于 {virushostdb_path}, 跳过下载和处理步骤\n"
            exit 0
        fi
...
rule genomad:
    input:
        votus = "votus_{type}/01_cluster/{sample}/votus.fa"
    output:
        dir = directory("votus_{type}/02-1_genomad/{sample}"),
        genomad_tax = "votus_{type}/02-1_genomad/{sample}/votus_annotate/votus_taxonomy.tsv"
    conda:
        "genomad"
    log:
        "votus_{type}/log/genomad/{sample}.log"
    shell:
        # 仅运行分类注释
...
rule mmseqs2:
    input:
        protein = "votus_{type}/02_prodigal/{sample}_protein.faa"
    output:
        nonredundant_protein = "votus_{type}/03_mmseqs2/{sample}_nonredundant_protein.faa"
    log:
        "votus_{type}/log/mmseqs2/{sample}.log"
    shell:
        """
...
rule diamond:
    input:
        nonredundant_protein = "votus_{type}/03_mmseqs2/{sample}_nonredundant_protein.faa"
    output:
        diamond_results = "votus_{type}/04_diamond/{sample}.tsv"
    log:
        "votus_{type}/log/diamond/{sample}.log"
    shell:
        """
        diamond blastp \
...
rule assign_classification:
    input:
        diamond_results = "votus_{type}/04_diamond/{sample}.tsv"
    output:
        #diamond_best_match = "votus_{type}/05_diamond_classification/{sample}/01_best_match.tsv",
        diamond_format = "votus_{type}/05_diamond_classification/{sample}/02_format.tsv",
        merge_diamond_refseq = "votus_{type}/05_diamond_classification/{sample}/03_merge.tsv",
        diamond_classification = "votus_{type}/05_diamond_classification/{sample}/04_classification.tsv"
    run:
        # 依次执行每个函数
...
rule merge_classification:
    input:
        ...
        genomad_tax = "votus_{type}/02-1_genomad/{sample}/votus_annotate/votus_taxonomy.tsv"
    output:
        merged_classification = "votus_{type}/06_merged_classification/{sample}.tsv"
    run:
        # 读取两个TSV文件（带表头）
...
rule postprocess:
    ...
    shell:
        "cp {input.merged_classification} {output.final_tax}"
