...
rule genomad:
    ...
    shell:
        # 仅运行分类注释
        "genomad annotate {input.virus} {output.dir} {genomad_db_path} > {log} 2>&1"
...
rule mmseqs2:
    input:
        protein = "uvigs_{type}/02_prodigal/{sample}_protein.faa"
    output:
        nonredundant_protein = "uvigs_{type}/03_mmseqs2/{sample}_nonredundant_protein.faa"
    log:
        "uvigs_{type}/log/mmseqs2/{sample}.log"
    shell:
        """
...
rule diamond:
    input:
        nonredundant_protein = "uvigs_{type}/03_mmseqs2/{sample}_nonredundant_protein.faa"
    output:
        diamond_results = "uvigs_{type}/04_diamond/{sample}.tsv"
    log:
        "uvigs_{type}/log/diamond/{sample}.log"
    shell:
        """
        diamond blastp \
...
rule assign_classification:
    input:
        diamond_results = "uvigs_{type}/04_diamond/{sample}.tsv"
    output:
        #diamond_best_match = "uvigs_{type}/05_diamond_classification/{sample}/01_best_match.tsv",
        diamond_format = "uvigs_{type}/05_diamond_classification/{sample}/02_format.tsv",
        merge_diamond_refseq = "uvigs_{type}/05_diamond_classification/{sample}/03_merge.tsv",
        diamond_classification = "uvigs_{type}/05_diamond_classification/{sample}/04_classification.tsv"
    run:
        # 依次执行每个函数
...
rule merge_classification:
    input:
        ...
        genomad_tax = "uvigs_{type}/02-1_genomad/{sample}/{sample}_uvigs_annotate/{sample}_uvigs_taxonomy.tsv"
    output:
        merged_classification = "uvigs_{type}/06_merged_classification/{sample}.tsv"
    run:
        # 读取两个TSV文件（带表头）
