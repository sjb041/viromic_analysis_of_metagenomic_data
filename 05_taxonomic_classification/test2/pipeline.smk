...
rule all:
    input:
        expand("04_assign_classification/{sample}_merge_taxonomy.tsv", sample=SAMPLES),
        expand("04_assign_classification/{sample}_final_taxonomy.tsv", sample=SAMPLES),
        
# 聚类
rule cluster:
    input:
        virus = os.path.join(virus_dir, "{sample}_uvigs.fa")
    output:
...
rule genomad:
    ...
    shell:
        ...
        "genomad annotate {input.votus} {output.dir} {genomad_db_path} --lenient-taxonomy > {log} 2>&1"
...
rule vcontact2:
    input:
        protein = "02_prodigal/{sample}_protein.faa"
    output:
        dir = directory("03_vcontact2/{sample}"),
        g2g = "03_vcontact2/{sample}/g2g.csv",
        overview = "03_vcontact2/{sample}/genome_by_genome_overview.csv"
    conda:
        "vc2"
    log:
        "log/vcontact2/{sample}.log"
    shell:
        """
...
rule assign_classification:
    input:
        ...
        overview = "03_vcontact2/{sample}/genome_by_genome_overview.csv"
    output:
        merge_taxonomy = "04_assign_classification/{sample}_merge_taxonomy.tsv",
        final_taxonomy = "04_assign_classification/{sample}_final_taxonomy.tsv"
    log:
        "log/assign_classification/{sample}.log"
    run:
...
onsuccess:
    ...
    shell(
        ...
        do
            (echo "---------------- ${{sample}} ----------------"; cat log/assign_classification/${{sample}}.log) >> {log_path}
        done
        cat {log_path}
        """,
