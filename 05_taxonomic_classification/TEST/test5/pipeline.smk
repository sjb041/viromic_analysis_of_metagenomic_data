...
rule all:
    input:
        expand("06_final/{sample}.tsv", sample=SAMPLES)
...
rule prodigal:
    input:
        renamed_fasta = "02_rename/{sample}_renamed.fa"
    output:
        protein = "03_prodigal/{sample}_protein.faa",
        count_orf = "03_prodigal/{sample}_count_orf.txt"
    log:
        "log/prodigal/{sample}.log"
    shell:
        """
        prodigal -i {input.renamed_fasta} -a {output.protein} -p meta > {log} 2>&1
...
rule hmmscan:
    input:
        protein = "03_prodigal/{sample}_protein.faa"
    output:
        tblout = "04_hmmscan/{sample}.tbl"
    log:
        "log/hmmscan/{sample}.log"
    shell:
        "hmmscan -E 0.001 --domE 0.1 --tblout {output.tblout} {hmmdb} {input.protein} > {log} 2>&1"
...
rule format:
    input:
        ...
        count_orf = "03_prodigal/{sample}_count_orf.txt"
    output:
        tblout_format = "04_hmmscan/{sample}_format.tbl"
    run:
...
rule assign:
    input:
        tblout_format = "04_hmmscan/{sample}_format.tbl"
    output:
        assign_tax = "05_assign/{sample}.tsv"
    run:
...
rule restore:
    input:
        ...
        assign_tax = "05_assign/{sample}.tsv"
    output:
        final_tax = "06_final/{sample}.tsv"
    run:
        mapping = input.mapping
        input_table = input.assign_tax
        output_table = output.final_tax
        
        # 读取新旧序列名映射文件
        name_df = pd.read_csv(mapping, sep='\t')
        # 创建映射字典
        name_mapping = dict(zip(name_df['renamed'], name_df['original_name']))

        # 读取要还原的表格
        df = pd.read_csv(input_table, sep='\t')

        # 将第一列的重命名序列还原为原始名称
        original_names = df.iloc[:, 0].map(name_mapping)

        # 检查是否存在无法映射的值
        if original_names.isna().any():
            # 找出无法映射的值
            unmapped_values = df.iloc[:, 0][original_names.isna()].tolist()
            raise ValueError(f"存在无法映射的序列名: {unmapped_values}")

        # 如果所有值都能映射，则更新DataFrame的第一列
        df.iloc[:, 0] = original_names
