---
title: "04_bray_curtis_dissimilarity_visualization"
...
getwd()
# 如果需要，可以改变工作目录
setwd("/home/shijiabin/2025_2ME/11_viral_community_types/A")
getwd()
```

```{r}
# 读取合并肠型分析结果的 phyloseq 对象
phyloseq_rarefied_phages_visualization_1 <- readRDS("03_community_types/phyloseq_rarefied_phages_visualization_1.rds")
```
```{r}
#################################### 1 数据预处理 ####################################
...
########### 把“组中心”直接画到 pcoa1 图上（强烈推荐）
# 用 Bray–Curtis 距离计算 group centroid
# 使用你已有的 Bray–Curtis 距离
bd <- betadisper(
  phyloseq_rarefied_phages_transformed_bray,
  group = meta_df$group
)

#bd

# 各 group 的中心点（在 PCoA 空间）
centroids <- bd$centroids
#centroids

# 计算 “组中心之间的距离矩阵”
centroid_dist <- dist(centroids, method = "euclidean")
cat("\nBray-Curtis distance between groups centroids:\n")
as.matrix(centroid_dist)

# 准备 centroid 数据用于 ggplot
centroid_df <- as.data.frame(centroids)
centroid_df$group <- rownames(centroid_df)

pcoa1_centroid <- pcoa1 +
  geom_point(
    data = centroid_df,
...
# ----------------------------
#ggsave(
#  filename = "GroupMean_BrayCurtis_heatmap.png",
#  plot = p_group_bray,
#  width = 6,
#  height = 5,
#  dpi = 300
...
cat("\nMean pairwise Bray–Curtis distance between groups:\n")
print(round(group_bray_mat, 3))
```
